...
> ⚠️ **Corrección (12-sep, verificado con investigación real):** Pollar corre sobre
> **Stellar**, no sobre Avalanche — no hay un puente confirmado entre las dos redes. La
> línea `Pollar->>Escrow: createAndFundOrder(...)` de este documento describe la
> arquitectura ideal a futuro, **no lo que la demo hace hoy**. Ver
> [`JUSTIFICACION-UNLOCK-POLLAR-CASOS-REALES.md`](JUSTIFICACION-UNLOCK-POLLAR-CASOS-REALES.md)
> y [`FLUJO-REAL-DEMO.md`](FLUJO-REAL-DEMO.md) para el flujo real: en la demo, el
> importador financia `TradeEscrow` directamente con USDC de testnet; Pollar se muestra
> como una integración **paralela y desacoplada** para cumplir su propio bounty. No decir
> en el pitch que Pollar deposita directo en el contrato — todavía no es cierto.
...
3. **Fondeo (visión a futuro, no implementado hoy):** el importador confirma al chofer y paga en Bolivianos con **QR Simple Pollar**. En la arquitectura ideal, esto financiaría `TradeEscrow.sol` — hoy, sin un puente confirmado entre Stellar (donde corre Pollar) y Avalanche (donde corre el escrow), el importador financia el contrato directamente con USDC de testnet, y Pollar se demuestra como un flujo de cobro real y separado.
4. **Tránsito y Telemetría:** El flete recorre los 1,200 km por Tambo Quemado.
5. **Liquidación Física (Tangem NFC):** El almacén receptor hace tap con su tarjeta Tangem de hardware (EAL6+). El contrato verifica la firma EIP-712 y libera el USDC custodiado directamente a la wallet del transportista verificado por Unlock.
...
sequenceDiagram
    ...
    Importador->>Pollar: Escanea QR Simple en BOB (Banco Unión/BCP) — flujo paralelo, red Stellar
    Importador->>Escrow: createAndFundOrder(Chofer, $2,500 USDC) — hoy en Avalanche, sin puente a Pollar
