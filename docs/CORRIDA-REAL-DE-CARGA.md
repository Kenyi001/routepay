...
> ⚠️ **Nota (12-sep, más tarde):** el deploy real ya se ejecutó por separado (broadcast
> con `--broadcast`, no solo la simulación de este doc). Las direcciones de
> `ERC2771Forwarder`/`TradeEscrow` de la sección 2 son una **predicción de simulación
> local** — pueden no coincidir con las del deploy real si se usó otra wallet o nonce.
> Verificar contra `frontend/src/contracts/addresses.ts` una vez actualizado, no contra
> este documento.
